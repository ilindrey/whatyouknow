...
from factory import LazyAttribute, Faker, PostGenerationMethodCall, SubFactory, post_generation, Iterator
...
class CategoryFactory(DjangoModelFactory):
    ...
    name = Iterator(['Technology', 'Development', 'Administration', 'Design', 'Business', 'Management', 'Marketing', 'Popular science'])
...
class PostFactory(DjangoModelFactory):
    class Meta:
        model = 'blog.Post'
        django_get_or_create = ('name', 'user', 'category',)

    user = SubFactory(UserProfileFactory)
    category = SubFactory(CategoryFactory)
    name = Faker('sentence')
    date = Faker('date_time_between', start_date='-2y', end_date='now', tzinfo=timezone.get_current_timezone())
    image = ImageField(width=4096, height=3072)
